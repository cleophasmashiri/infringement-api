...
class VehicleGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Vehicle entity")
        ...
        .repeat(2) {
            ...
            .get("/services/infringementapi/api/vehicles")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new vehicle")
            .post("/services/infringementapi/api/vehicles")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "plateNumber":"SAMPLE_TEXT"
                , "make":"SAMPLE_TEXT"
                , "model":"SAMPLE_TEXT"
                , "engineNumber":"SAMPLE_TEXT"
                , "chassisNumber":"SAMPLE_TEXT"
                , "color":"SAMPLE_TEXT"
                , "yearFirstRegistered":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            ...
            .repeat(5) {
                ...
                .get("/services/infringementapi${new_vehicle_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created vehicle")
            .delete("/services/infringementapi${new_vehicle_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
